    ...
    try {
      if (ans == "yes") {
        const res = await axios.delete(
          `/api/reservations/deleteBooking/${data._id}`
        );
        navigate("/");
    ...
    <div className="min-h-[170px] w-[90%]  md:w-[80%] mx-auto  p-5 px-6 bg-white border-[0.5px] border-solid rounded-sm border-gray-300 flex justify-between ">
      <div className="flex flex-col  ">
        <h1 className="uppercase font-semibold ">
          {data.name}'<span className="lowercase">s</span>
        </h1>
      ...
      <div className="flex flex-col justify-center">
        <p>
          <b>In</b>: {data.checkIn.split("T")[0]}
        </p>
        <p>
          <b>Out</b>: {data.checkIn.split("T")[0]}
        </p>
      </div>
      <div className="flex items-center">
        <button className=" rounded-sm bg-[#d11242] text-white px-3 py-2 ">
          Pay &nbsp;₹{data.price}/-{" "}
        </button>
      </div>
      {openModal ? (
